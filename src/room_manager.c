...
#define MAX_KEY_ROOMS 8  // Maximum number of potential key rooms per stage

// Structure to track potential key rooms
typedef struct {
    UINT8 key_room_ids[MAX_KEY_ROOMS];
    UINT8 num_key_rooms;
} KeyRoomTracker;

// Global variables
UINT8 selected_key_room = 255;
KeyRoomTracker current_key_tracker;  // Make it global for debug access

void find_key_rooms(const LevelTemplate* template, KeyRoomTracker* tracker) {
    tracker->num_key_rooms = 0;
    
    // Scan through all rooms in template
    for(UINT8 i = 0; i < template->num_rooms; i++) {
        if(template->rooms[i].type == ROOM_TYPE_KEY) {
            // Add this room's ID to our tracker
            if(tracker->num_key_rooms < MAX_KEY_ROOMS) {
                tracker->key_room_ids[tracker->num_key_rooms] = i;
                tracker->num_key_rooms++;
            }
        }
    }
}

void select_random_key_room(KeyRoomTracker* tracker) {
    if(tracker->num_key_rooms > 0) {
        // Get more entropy by waiting
        wait_vbl_done();
        UINT8 div_value = DIV_REG;
        wait_vbl_done();
        UINT8 time_value = TIMA_REG;
        
        // Combine values to create more randomness
        UINT8 mixed_value = div_value ^ time_value;
        
        // Use this for selection
        UINT8 random_index = mixed_value % tracker->num_key_rooms;
        
        selected_key_room = tracker->key_room_ids[random_index];
        
        // Force a wait to ensure values change
        wait_vbl_done();
    } else {
        selected_key_room = 255;
    }
}
...
void init_stage(UINT8 stage_number) {
    ...
    // Force some waits to ensure randomness
    wait_vbl_done();
    wait_vbl_done();
    
    // Find all potential key rooms first
    find_key_rooms(template, &current_key_tracker);
    
    // Select random key room BEFORE room generation
    select_random_key_room(&current_key_tracker);
    
    // Now proceed with room generation using selected_key_room
    ...
    // Set initial room and other initialization
