...
#include "text.h"
#include "sound.h"
#include "sequencer.h"  // Add this include
...
static void update_note_display(void) {
   for(UINT8 i = 0; i < 4; i++) {
       draw_text(14, 2 + i, "   ");
       draw_text(14, 2 + i, NOTE_NAMES[sequencer.steps[sequencer.current_step].note]);
   }
}
...
static void update_tempo_display(void) {
   ...
   sprintf(buffer, "♪%d %c", sequencer.tempo, 
           sequencer.is_playing ? '\x1A' : '\x1B');
   draw_text(0, 0, buffer);
}

static void update_playback(void) {
   if (sequencer.is_playing && sequencer.steps[sequencer.playback_step].armed) {
       UINT8 note = sequencer.steps[sequencer.playback_step].note;
       play_sequencer_note(note);
   } else {
       stop_sequencer_note();
   }
}
...
void init_sequencer(void) {
   ...
   sequencer.current_channel = 0;
   calculate_frames_per_step();
   
   for (UINT8 i = 0; i < SEQ_MAX_STEPS; i++) {
       sequencer.steps[i].armed = 0;
       sequencer.steps[i].note = 24;  // Default to C5 (middle C)
...
void draw_sequencer(void) {
   ...
   draw_text(14, 2, NOTE_NAMES[sequencer.steps[sequencer.current_step].note]);
  
   draw_parameter("C2", 12, 3);
   draw_text(14, 3, NOTE_NAMES[sequencer.steps[sequencer.current_step].note]);
  
   draw_parameter("C3", 12, 4);
   draw_text(14, 4, NOTE_NAMES[sequencer.steps[sequencer.current_step].note]);
  
   draw_parameter("C4", 12, 5);
   draw_text(14, 5, NOTE_NAMES[sequencer.steps[sequencer.current_step].note]);
    ...
    if(joy != last_joy) {
        ...
        if(sequencer.in_grid_mode) {
            ...
            if(needs_step_update) {
                ...
                update_note_display();  // Only update note display instead of full redraw
                draw_step_status();     // Update step status
            }
            ...
            if(joy & J_LEFT || joy & J_RIGHT) {
                ...
                switch(sequencer.cursor) {
                    ...
                    case PARAM_CH3:
                    case PARAM_CH4: {
                        UINT8 new_note = sequencer.steps[sequencer.current_step].note + delta;
                        if(new_note <= SEQ_MAX_NOTE && new_note >= SEQ_MIN_NOTE) {
                            sequencer.steps[sequencer.current_step].note = new_note;
                            // Clear space first, then update
                            draw_text(14, 2 + sequencer.cursor, "   ");
                            draw_text(14, 2 + sequencer.cursor, NOTE_NAMES[new_note]);
                        }
                        break;
                    }
                    case PARAM_TEMPO: {
